...
const  registerUser= asyncHandler(async (req,res, next)=>{
    ...
    return res.status(201).json(
        new ApiResponse(200, {user: createdUser},"User registered successfully and verification has been sent on your email")
    )
})
...
const logoutUser = asyncHandler(async (req,res)=>{
    await User.findByIdAndUpdate(
        ...
        {
                new:true //onece everything is done , it gives the most updated object
        },
    )
    const options = {
        httpOnly: true,
...
const verifyEmail = asyncHandler(async(req,res)=>{
    ...
    let hashedToken = crypto.createdHash("sha256").update(verificationToken).digest("hex")
